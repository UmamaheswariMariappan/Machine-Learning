...
class ModelTrainer:
    ...
    def get_model_object_and_report(self, train: np.array, test: np.array) -> Tuple[object, object]:
        ...
        try:
            ...
            # Convert x_test to DataFrame if possible for logging
            
            # Save x_test DataFrame info for automated comparison
         
            if not isinstance(x_test, pd.DataFrame):
                x_test_df = pd.DataFrame(x_test)
            else:
                x_test_df = x_test
            x_test_df.columns.to_series().to_csv("train_df_columns.csv")
            x_test_df.dtypes.to_csv("train_df_dtypes.csv")
            pd.DataFrame(x_test_df.values).to_csv("train_df_values.csv", index=False)

            y_pred = model_obj.predict(x_test)
            # Print first 20 true and predicted labels
